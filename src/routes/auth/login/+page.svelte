<script lang="ts">
	import { Button, Card, CardHeader, CardTitle, CardDescription, CardContent } from '$lib/components/ui'
	import { signInWithGoogle, authState } from '$lib/auth.svelte'
	import { goto } from '$app/navigation'
	import { LogIn } from 'lucide-svelte'
	
	let loading = $state(false)
	let error = $state<string | null>(null)

	async function handleGoogleSignIn() {
		loading = true
		error = null

		try {
			await signInWithGoogle()
			goto('/dashboard')
		} catch (err: any) {
			error = err.message || 'Login failed. Please try again.'
		} finally {
			loading = false
		}
	}

	// Redirect if already logged in
	$effect(() => {
		if (authState.user && !authState.loading) {
			goto('/dashboard')
		}
	})
</script>

<svelte:head>
	<title>Login - Open-EDU</title>
	<meta name="description" content="Sign in to your Open-EDU account" />
</svelte:head>

<div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
	<div class="max-w-md w-full space-y-8">
		<!-- Header -->
		<div class="text-center">
			<LogIn class="mx-auto h-12 w-12 text-primary" />
			<h2 class="mt-6 text-3xl font-bold text-foreground">Welcome to Open-EDU</h2>
			<p class="mt-2 text-sm text-muted-foreground">
				Sign in with your Google account to start learning
			</p>
		</div>

		<!-- Login Card -->
		<Card>
			<CardHeader>
				<CardTitle>Sign In</CardTitle>
				<CardDescription>
					Use your Google account to access Open-EDU
				</CardDescription>
			</CardHeader>
			<CardContent>
				<!-- Error Message -->
				{#if error}
					<div class="bg-destructive/10 border border-destructive/20 rounded-md p-3 mb-6">
						<p class="text-sm text-destructive">{error}</p>
					</div>
				{/if}

				<!-- Google Sign In Button -->
				<Button 
					onclick={handleGoogleSignIn}
					class="w-full flex items-center justify-center gap-3 py-3" 
					disabled={loading}
					variant="outline"
				>
					{#if loading}
						<div class="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin"></div>
						Signing in...
					{:else}
						<svg class="w-5 h-5" viewBox="0 0 24 24">
							<path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
							<path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
							<path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
							<path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
						</svg>
						Continue with Google
					{/if}
				</Button>

				<!-- Info Section -->
				<div class="mt-6 pt-6 border-t border-border">
					<div class="text-center text-sm text-muted-foreground space-y-2">
						<p>🎓 Start your learning journey today</p>
						<p>✨ Access thousands of courses</p>
						<p>📱 Learn anywhere, anytime</p>
					</div>
				</div>
			</CardContent>
		</Card>
	</div>
</div>