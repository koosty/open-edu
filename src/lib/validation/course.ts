...
export const quizQuestionSchema = z.object({
	...
	if (data.type === 'multiple_choice' && Array.isArray(data.options)) {
		...
		return !isNaN(answerIndex) && answerIndex >= 0 && answerIndex < data.options.length
	}
	return true
}, {
	message: 'Correct answer must be a valid option index for multiple choice questions',
	path: ['correctAnswer']
})
...
export const chapterSchema = z.object({
	id: z.string(),
	courseId: z.string(),
	title: z.string().min(1, 'Chapter title is required').max(200, 'Title cannot exceed 200 characters'),
...
export const courseSchema = z.object({
	...
	learningOutcomes: z.array(z.string().min(1)).min(1, 'Must have at least one learning outcome').max(10, 'Cannot have more than 10 learning outcomes'),
	createdAt: z.string(),
	updatedAt: z.string(),
...
export const courseSortSchema = z.object({
	field: z.enum(['title', 'createdAt', 'rating', 'enrolled', 'price', 'updatedAt']),
