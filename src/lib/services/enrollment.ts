...
export class EnrollmentService {
	...
	static async hasAccess(userId: string, courseId: string): Promise<boolean> {
		try {
			const enrollment = await this.getEnrollment(userId, courseId)
			return enrollment !== null && ['enrolled', 'active', 'completed'].includes(enrollment.status)
		} catch (error) {
	 ...
	 */
	private static async createInitialProgress(userId: string, courseId: string): Promise<void> {
		try {
			const progressData = {
				userId,
				courseId,
	 ...
	 */
	private static async updateProgress(userId: string, courseId: string, updates: any): Promise<void> {
		try {
			const q = query(
	 ...
	 */
	static async issueCertificate(userId: string, courseId: string, certificateUrl: string): Promise<void> {
		try {
			const enrollment = await this.getEnrollment(userId, courseId)
			if (!enrollment) {
				throw new Error('User is not enrolled in this course')
