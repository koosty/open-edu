...
export class ProgressService {
	...
	static async completeLesson(
		...
		try {
			...
			const newProgressPercentage = totalLessons > 0 ? (completedLessons / totalLessons) * 100 : 0
			...
			if (quizScore !== undefined) {
				...
				const averageQuizScore = allScores.reduce((sum, score) => sum + score, 0) / allScores.length
	...
	static async updateQuizAttempt(
		...
		try {
			...
			if (score > currentBestScore) {
				...
				const averageQuizScore = allScores.reduce((sum, s) => sum + s, 0) / allScores.length
				updates.averageQuizScore = Math.round(averageQuizScore * 10) / 10
			}
	 ...
	 */
	static async isLessonCompleted(userId: string, courseId: string, lessonId: string): Promise<boolean> {
		try {
			const progress = await this.getCourseProgress(userId, courseId)
	 ...
	 */
	static async getQuizScore(userId: string, courseId: string, lessonId: string): Promise<number | null> {
		try {
			const progress = await this.getCourseProgress(userId, courseId)
	 ...
	 */
	static async getProgressSummary(userId: string, courseId: string): Promise<{
		...
		try {
			...
			const course = courseSnap.exists() ? 
				{ id: courseSnap.id, ...convertTimestamps(courseSnap.data()) } as Course : 
				null
			...
			if (progress && course?.lessons) {
				...
				completionRate = (progress.completedLessons.length / course.lessons.length) * 100
	 ...
	 */
	static async updateActivityStreak(userId: string, courseId: string): Promise<void> {
		try {
			const progress = await this.getCourseProgress(userId, courseId)
			...
			const now = new Date()
			const lastActiveDate = new Date(progress.lastActiveDate || progress.lastAccessedAt)
