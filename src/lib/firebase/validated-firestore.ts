...
export class ValidatedFirestore {
	...
	async addValidated<T>(
		...
		try {
			...
			const docRef = await addDoc(collection(this.db, collectionName), validated as any)
			return docRef.id
		} catch (error: any) {
			const isZodError = error.name === 'ZodError'
			throw new ValidationError(
	...
	async updateValidated<T>(
		...
		try {
			...
			await updateDoc(doc(this.db, collectionName, docId), updatesWithTimestamp as any)
		} catch (error: any) {
			throw new ValidationError(
...
export function createTypedService<T>(
	...
	return {
		...
		async update(id: string, updates: Partial<T>): Promise<void> {
			return validatedFirestore.updateValidated(collectionName, id, updates, schema)
		},
		...
		async query(...constraints: QueryConstraint[]): Promise<T[]> {
			return validatedFirestore.queryValidated(collectionName, schema, ...constraints)
		},
