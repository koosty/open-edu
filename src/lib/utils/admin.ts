 ...
 */
export function canManageCourse(courseInstructorId: string, user?: User | null): boolean {
	const currentUser = user ?? authState.user
 ...
 */
export function getUserAccessLevel(user?: User | null): 'admin' | 'instructor' | 'student' {
	const currentUser = user ?? authState.user
